...
export class Game {
  ...
  @Prop()
  reference!: string;
  @Prop({ type: MoundVisitsSchema, required: false })
  mound_visits?: MoundVisits;
  @Prop({ type: ReviewsSchema, required: false })
  reviews?: Reviews;
  @Prop({ type: TimeZonesSchema, required: false })
  time_zones?: TimeZones;
  @Prop({ type: VenueSchema, required: false })
  venue?: Venue;
  @Prop({ type: [BroadcastSchema], default: [] })
  broadcasts!: Broadcast[];
  @Prop({ type: WeatherSchema, required: false })
  weather?: Weather;
  @Prop({ type: FinalInfoSchema, required: false })
  final?: FinalInfo;
  @Prop({ type: TeamStatusSchema, required: false })
  home?: TeamStatus;
  @Prop({ type: TeamStatusSchema, required: false })
  away?: TeamStatus;
  @Prop({ type: PitchingSchema, required: false })
  pitching?: Pitching;
}
